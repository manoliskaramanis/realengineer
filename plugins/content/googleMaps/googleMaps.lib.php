...
* v3.0.22 Apr 16, 2014 changed setup_css_joomla3($map_id) to setup_css_joomla3($map_id) so that compatible with googleDirections. Also added one more parameter "with jQuery library"
* v3.0.23 Dec 16, 2014 changed googleMaps.xml - template_uses_jquery
* For joomla 1.0, use back onload_{$this->mod}(load_{$this->mod})

* Aug 23, 2014 added labelClass: "googleMaps_label" to allow labels to be modified using css
*/
...
class Plugin_googleMaps_base {
	...
	function init_google_maps($row, $pluginParams, $is_mod) {
		...
		$param_list = array('api_key', 'width', 'height', 'zoom', 'template_uses_jquery');
		...
		if (preg_match_all( $regex, $contents, $matches, PREG_SET_ORDER )) {
			...
			foreach($matches as $matches2) {
				...
				if(defined('_VALID_MOS')) {
					...
					JFactory::getDocument()->addStyleDeclaration($this->setup_css_joomla3($map_id));
				}
			...
			global $joomla_ver;
			if (!isset($joomla_ver)) $joomla_ver = '2.5';
			#print "bp301. joomla_ver = $joomla_ver<br>";
			#print "bp301.1. template_uses_jquery = $this->template_uses_jquery<br>";

			#if ($joomla_ver=='3.0') {
			if ($this->template_uses_jquery) {
...
//window.addEvent('load', load_{$this->mod});
                #print "bp121. joomla_ver = $joomla_ver<br>";
                if ($joomla_ver == '1.0') {
                	$load_gmap_script = "onload_{$this->mod}(load_{$this->mod});";
                } else {
...
";
	...
	function setup_css_joomla3($map_id) {
